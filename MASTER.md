...
- Grimpe autonome corrigée et re-validée le 2026-09-17 : l'état `run` était un cul-de-sac (le chat courait en boucle, ennui bloqué à 0, grimpe jamais déclenchée) ; `run` est désormais re-décidable. Précision améliorée : bornes visibles exactes via `DWMWA_EXTENDED_FRAME_BOUNDS` (le chat était décalé de ~7 px à cause des bordures invisibles DWM). Grimpe complète et chute à la fermeture observées par l'utilisateur. Sauts entre fenêtres, hop vers le sol et suivi au déplacement restent à re-observer sur la durée.
